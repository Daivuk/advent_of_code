...
int calculate1()
{
    ...
    int previous = std::stoi(lines.front());
    for (int i = 1; i <  (int)lines.size(); ++i)
    {
        auto current = std::stoi(lines[i]);
...
int calculate2()
{
    ...
    {
        int previous = std::stoi(lines[i - 3]) + std::stoi(lines[i - 2]) + std::stoi(lines[i - 1]);
        int current  = std::stoi(lines[i - 2]) + std::stoi(lines[i - 1]) + std::stoi(lines[i]);
        if (current > previous) increase_count++;
    }
    return increase_count;
}
